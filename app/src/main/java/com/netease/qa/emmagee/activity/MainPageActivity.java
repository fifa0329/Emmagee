...
import com.netease.qa.emmagee.utils.Program;
...
public class MainPageActivity extends Activity {
	...
	private List<Program> processList;
	...
	private void waitForAppStart(String packageName) {
		...
		while (System.currentTimeMillis() < startTime + TIMEOUT) {
			...
			for (Program programe : processList) {
	...
	private class ListAdapter extends BaseAdapter {
		...
		public ListAdapter() {
			mPrograms = processInfo.getRunningProcess(getBaseContext());
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			Program pr = mPrograms.get(position);
			if (convertView == null)
		...
		OnCheckedChangeListener checkedChangeListener = new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					checkedProg = mPrograms.get(checkedPosition);
